...
- **Hybrid Comms Restore (archivist_ai_0)** - Deployed cadence tools (`tools/end_of_block.py`, heartbeat/offline sync/ops readiness/exchange_all) and factory order emitter into tools/, removed old copies, and shifted smoke to `python tools/factory_order_emitter.py --help` to confirm emitter presence.
