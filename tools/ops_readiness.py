...
def check_files(paths):
    return {str(p): Path(p).exists() for p in paths}
...
def main():
    hub = read_hub_path()
    files_expect = [ROOT/"RUNBOOK.md"]  # keep minimal here
    file_status = check_files(files_expect)
    ...
    summary = {
        ...
        "hub_exists": hub_ok,
        "docs_present": file_status,
        "staged_checks": staged_checks,
        "ok": hub_ok and all(file_status.values()) and not has_missing,
    }
